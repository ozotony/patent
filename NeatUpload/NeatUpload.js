...
function NeatUploadForm(formElem, postBackID)
{
	...
	SubmitCount = 0;
	FormElem = formElem;
	TriggerIDs = new Object();
	TriggerIDs.NeatUpload_length = 0;
	OnNonuploadHandlers = new Array();
	GetFileSizesCallbacks = new Array();
	
	// Add a hook to call our own unload handler(s) which do things like restore the original on submit handlers
	OnUnloadHandlers = new Array();	
	var origOnUnload = window.onunload;
	OnUnloadHandlers.push(function () { window.onunload = origOnUnload; });
	window.onunload = CombineHandlers(window.onunload, function() { return f.OnUnload(); });

	// Override the form.submit() method to call our own handlers before and after.
	debugMessage("overriding form.submit()");
	f.FormElem.NeatUpload_OnSubmittingHandlers = new Array();
	f.FormElem.NeatUpload_OnSubmitting = OnSubmitting;
	OnUnloadHandlers.push(function() 
	{
	...
	f.FormElem.NeatUpload_OnSubmit = OnSubmit;
	try
	{
	...
	catch (ex)
	{
		...
		debugMessage("can't override form.submit()");
	}			
	...
	try
	{
		...
		Event.prototype.preventDefault = function () {
			NeatUpload_PreventDefaultCalled = true;
	     returnValue = false; // For IE 8, which doesn't define preventDefault but does allow it to be set.
			if (NeatUpload_OrigPreventDefault)
				return NeatUpload_OrigPreventDefault();
		};
		debugMessage("Hooked preventDefault");
	}
	catch (ex)
	{
		debugMessage("Could not hook preventDefault: " + ex);
	}
	...
	debugMessage("hooking form.onsubmit()");
	    ...
	    window.setTimeout(function() {
	        ...
	        if (f.FormElem.NeatUpload_OrigOnSubmit) {
	            ...
	            f.FormElem.onsubmit = function(oev) {
	                ...
	                var returnValue = NeatUpload_OrigOnSubmit(ev);
	                if (ev)
	...
	debugMessage("Adding handlers for possible triggers");
	...
	for (var i = 0; i < eventsThatCouldTriggerPostBack.length; i++)
	{
		...
		AddHandler(f.FormElem, eventName, function(ev) {
	...
	debugMessage("Adding submitting handler");	
	AddSubmittingHandler(function () {
	...
	debugMessage("Submitting handler added");
...
{
	return PostBackID + SubmitCount.toString();
}
...
{
	TriggerIDs[id] = ++TriggerIDs.NeatUpload_length;
...
NeatUploadForm.prototype.AddHandler = function(elem, eventName, handler, useCapture)
{
	...
	if (elem.addEventListener)
	{
		...
		OnUnloadHandlers.push(function () { elem.removeEventListener(eventName, handler, useCapture); });
	}
	else if (elem.attachEvent)
	{
		elem.attachEvent("on" + eventName, handler);
		OnUnloadHandlers.push(function () { elem.detachEvent("on" + eventName, handler); });
	}
	else
	{
		var origHandler = elem["on" + eventName];
		elem["on" + eventName] = CombineHandlers(elem["on" + eventName], handler);
		OnUnloadHandlers.push(function () { elem["on" + eventName] = origHandler; });
	}
};
...
{
	for (var i=0; i < NeatUpload_OnSubmittingHandlers.length; i++)
	{
		NeatUpload_OnSubmittingHandlers[i].call(this);
	}
};
...
NeatUploadForm.prototype.OnSubmit = function(ev) {
    ...
    var retVal = true;
    var asyncHandlers = [];
    for (var i = 0; i < NeatUpload_OnSubmitHandlers.length; i++) {
        var status = NeatUpload_OnSubmitHandlers[i].call(this, ev);
        if (status === false)
            return false;
...
{
	OnNonuploadHandlers.push(handler);
};
...
NeatUploadForm.prototype.OnNonupload = function(elem)
{
	...
	for (var i=0; i < OnNonuploadHandlers.length; i++)
	{
		OnNonuploadHandlers[i].call(elem);
	}
};

NeatUploadForm.prototype.AddGetFileSizesCallback = function(callback)
{
	GetFileSizesCallbacks.push(callback);
};

NeatUploadForm.prototype.GetFileSizes = function(elem)
{
	var fileSizes = [];
	for (var i=0; i < GetFileSizesCallbacks.length; i++)
	{
		fileSizes = fileSizes.concat(GetFileSizesCallbacks[i].call(elem));
	}
...
function NeatUploadPB(id, postBackID, uploadProgressPath, popupWidth, popupHeight, triggerIDs, autoStartCondition)
{
	...
	UploadForm = NeatUploadForm.prototype.GetFor(elem, postBackID);
	debugMessage("GetFor returned");
	...
	TriggerIDs = new Object();
	TriggerIDs.NeatUpload_length = 0;
	AutoStartCondition = autoStartCondition;

	UploadForm.AddNonuploadHandler(function () { pb.ClearFileInputs(pb.UploadForm.FormElem); });

	UploadForm.AddSubmitHandler(function (ev) {
	...
	{
		UploadForm.AddTrigger(triggerIDs[i]);
		TriggerIDs[triggerIDs[i]] = ++TriggerIDs.NeatUpload_length;
	}
	debugMessage("NeatUploadPB returning");
}
...
NeatUploadPB.prototype.Display = function() {
	DisplayUrl(UploadProgressPath + '&postBackID=' + UploadForm.GetPostBackID() + '&refresher=client&canScript=true&canCancel=' + NeatUploadPB.prototype.CanCancel());
};
...
{
	var isFilesToUpload = (UploadForm.GetFileSizes().length > 0);
		...
		if (inputFile.type == 'file' && inputFile.value && inputFile.value.length > 0)
		{
			...
			catch (ex)
			{
				...
				if (inputFile.value != null && inputFile.value != '')
				{
					...
					{
						window.alert(ClearFileNamesAlert);
							...
							storageConfigFieldName)
{
	...
	ClientID = clientID;
	AppPath = appPath;
	PostBackIDQueryParam = postBackIDQueryParam;
	UploadScript = uploadScript;
	UploadParams = uploadParams;
	FilesToUpload = [];
	FileID = 0;
	FileQueueControlID = fileQueueControlID;
	StorageConfigFieldName = storageConfigFieldName;
	...
	GetInputFileElem().onchange = function(ev) {
		...
		removeAttribute("id");
		parentNode.insertBefore(newInputFile, nextSibling);
		style.display = 'none';
		style.position = 'relative';
		FileQueued({ name: value, size: -1, inputFileElem: this, id: numf.FileID++});		
        return true;
	};	
	...
	GetInputFileElem().parentNode.insertBefore(fqc, GetInputFileElem());
	...
	function FileQueued(file) {
		...
		file.Delete = function() {
		    if (numf.IsFlashLoaded && numf.Swfu)
		    {
				numf.Swfu.cancelUpload(id);
			}
			else
			{
				inputFileElem.parentNode.removeChild(inputFileElem);
				FileCancelled(this);
			}
		}
...
{
	if (typeof(FileQueueControlID) == "string" && FileQueueControlID.length > 0)
	{
		fqc = document.getElementById(FileQueueControlID);
	}
	return fqc;
};
